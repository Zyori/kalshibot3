...
function TotalGoalBody({ ticker, snapshot }: { ticker: string; snapshot: TotalGoal }) {
  ...
  // Seed the cache from the event-endpoint snapshot once, so the panel opens
  // with a real top-of-book before the first WS delta lands. Only the two best
  // prices are known here (no depth), which is enough for the panel's auto-
  // follow; WS deltas then own the book. No-op once a book exists.
